(() => {
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) {
            const sloganNode = document.querySelectorAll(".hero-page__text span");
            let menu = [];
            for (let index = 0; index < sloganNode.length; index++) {
                const element = sloganNode[index];
                menu.push(element.innerHTML);
            }
            new swiper_core_Swiper(".hero-page__slider ", {
            ...
            new swiper_core_Swiper(".testimonials-page__slider", {
                ...
                on: {
                    slideChange: () => {
                        const moreBtns = document.querySelectorAll(".text-testimonials__more");
                        moreBtns.forEach((moreBtn => {
                            const spans = moreBtn.querySelectorAll("span");
                            const lastSpan = spans[spans.length - 1];
                            const computedStyle = window.getComputedStyle(lastSpan);
                            const displayValue = computedStyle.getPropertyValue("display");
                            if (displayValue !== "none") simulateClick(moreBtn);
                        }));
                    }
                }
            });
...
***************************************************************************** */
    ...
    let formIsInserted = false;
    document.addEventListener("click", (event => {
        const clickedElement = event.target;
        if (clickedElement.dataset.imgPath) {
            const popupImg = popupShowImage.querySelector(".popup__img-ibg img");
            popupImg.src = clickedElement.dataset.imgPath;
        } else if (clickedElement.classList.contains("popup-button")) console.log("Popup button clicked"); else formIsInserted == false;
    }));
    document.addEventListener("DOMContentLoaded", (() => {
        const pageImgs = document.querySelectorAll(".page-img__img");
        const contentButtons = document.querySelectorAll(".content__button");
        if (window.innerHeight < 670) contentButtons.forEach((button => {
            if (!button.classList.contains("content__button_no-mb")) button.style.marginBottom = "30px";
        }));
        pageImgs.forEach((img => {
            if (img.clientHeight < 60) img.style.display = "none";
        }));
    }));
    const reforms = document.querySelector(".reforms");
    if (reforms && !isMobile.any()) {
        const reformsItems = document.querySelector(".reforms__items");
        const reformsColumn = document.querySelectorAll(".reforms__column");
        const speed = reforms.dataset.speed;
        let positionX = 0;
        let coordXprocent = 0;
        function setMouseGalleryStyle() {
            let reformsItemsWidth = 0;
            reformsColumn.forEach((element => {
                reformsItemsWidth += element.offsetWidth;
            }));
            const reformsDifferent = reformsItemsWidth - reforms.offsetWidth;
            const distX = Math.floor(coordXprocent - positionX);
            positionX += distX * speed;
            let position = reformsDifferent / 200 * positionX;
            reformsItems.style.cssText = `transform: translate3d(${-position}px,0,0);`;
            if (Math.abs(distX) > 0) requestAnimationFrame(setMouseGalleryStyle); else reforms.classList.remove("_init");
        }
        reforms.addEventListener("mousemove", (function(e) {
            const reformsWidth = reforms.offsetWidth;
            const coordX = e.pageX - reformsWidth / 2;
            coordXprocent = coordX / reformsWidth * 200;
            if (!reforms.classList.contains("_init")) {
                requestAnimationFrame(setMouseGalleryStyle);
                reforms.classList.add("_init");
            }
        }));
    }
    const projectContainer = document.querySelector(".gallery__imgs");
    const galleryTitle = document.querySelector(".gallery__title");
    const reformItems = document.querySelectorAll(".reforms__item");
    const urlParams = new URLSearchParams(window.location.search);
    const projectId = urlParams.get("id");
    const projectTitles = document.querySelectorAll(".item-reforms__title");
    const projectPhotos = {};
    function createProjectId(titles, items) {
        const ids = [];
        let i = 0;
        titles.forEach((title => {
            const projectTitle = title.innerText.toLowerCase().replace(" ", "-");
            projectPhotos[`${projectTitle}`] = [];
            for (let index = 1; index <= 5; index++) projectPhotos[`${projectTitle}`].push(`img/reforms/gallery/${projectTitle}/0${index}.jpg`);
            ids.push(projectTitle);
        }));
        items.forEach((item => {
            item.href = `./gallery.html?id=${ids[i]}`;
            i++;
        }));
    }
    if (window.location.href.includes("reforms.html")) {
        createProjectId(projectTitles, reformItems);
    ...
    if (projectId && projectPhotos1[projectId]) {
        galleryTitle.innerHTML = `${projectId.replace("-", " ")}`;
        const photos = projectPhotos1[projectId];
        photos.forEach((photoSrc => {
            const img = createImageElement(photoSrc);
